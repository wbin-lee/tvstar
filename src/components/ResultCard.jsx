...
export default function ResultCard({ character, onRestart }) {
  ...
  const name = character.name?.[lang] || character.name?.en || character.name;
  const desc = character.description?.[lang] || character.description?.en || character.description;

  return (
    <div className="animate-fade-in max-w-md mx-auto text-center">
      <h2 className="text-2xl font-bold mb-6">{t.youAre}</h2>
      <div className="rounded-2xl overflow-hidden bg-gray-900 border border-gray-800 shadow-lg shadow-indigo-500/10">
        ...
        />
        <div className="p-6">
          <h3 className="text-2xl font-bold text-indigo-400">{name}</h3>
